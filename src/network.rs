...
use ureq::{Agent, AgentBuilder, Error as UreqError, Transport, ErrorKind as UreqErrorKind};
...
impl NetworkChecker {
    ...
    fn try_connect(&self, domain: &str) -> AppResult<ConnectionResult> {
        ...
        match request.call() {
                ...
                if initial_url != final_url {
                    ...
                    if initial_domain != final_domain {
                        if is_block_page_url(&final_url) {
                            println!("DEBUG: Detected redirect to potential block page: {}", final_url);
            ...
            Err(_) => {
                println!("DEBUG: Connection failed");
...
fn handle_transport_error(transport: &Transport) -> AppResult<ConnectionResult> {
    match transport.kind() {
        UreqErrorKind::Io => {
            ...
            } else if err_string.contains("connection refused") || err_string.contains("connection reset") || err_string.contains("connection aborted") {
                return Ok(ConnectionResult::ConnectionReset);
            } else if err_string.contains("not found") || err_string.contains("name resolution failed") || err_string.contains("no such host") {
...
fn extract_domain(url: &str) -> String {
    ...
    without_protocol.split('/').next().unwrap_or(without_protocol).to_string()
}
...
impl DPITestResult {
    pub fn to_russian_string(&self) -> String {
        match self {
            ...
            DPITestResult::ISPBlocked => "Сайт заблокирован провайдером".to_string().to_uppercase(),
