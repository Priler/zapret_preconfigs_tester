...
use sysinfo::{RefreshKind, System, Process, ProcessRefreshKind, ProcessesToUpdate};
...
impl ProcessManager {
    pub fn new() -> Self {
        Self {
            sys: System::new_with_specifics(RefreshKind::new().with_processes(ProcessRefreshKind::everything()))
        }
    }
    ...
    pub fn wait_for_process(&mut self, process_name: &str, timeout: Duration) -> bool {
        ...
        while start.elapsed() < timeout {
            ...
            if self.sys.processes().values().any(|process| process.name() == process_name) {
                return true;
            }
    ...
    pub fn ensure_process_terminated(&mut self, process_name: &str) {
        for _ in 0..3 {  // Try up to 3 times
            self.sys.refresh_processes(ProcessesToUpdate::All, true);
            let processes: Vec<_> = self.sys.processes()
                .values()
